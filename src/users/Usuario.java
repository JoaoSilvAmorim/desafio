...
public class Usuario {
  ...
  public void addUser(DirContext connection) {
    ...
    String aux = "cn=" + this.login + ",ou=Usuario,dc=ramhlocal,dc=com";
        ...
        if (grupos != null && !grupos.isEmpty()) {
          for (String grupo : grupos) {
            if (groupExists(grupo, connection)) {
              ...
              System.out.println("O grupo: " + grupo + " não existe");
          }
        }
  ...
  public void addUserToGroup(String login, String groupName, DirContext connection) {
    ...
    Attribute attribute = new BasicAttribute("member", "cn=" + login + ",ou=Usuario,dc=ramhlocal,dc=com");
    mods[0] = new ModificationItem(DirContext.ADD_ATTRIBUTE, attribute);
    try {
      connection.modifyAttributes("cn=" + groupName + ",ou=Grupo,dc=ramhlocal,dc=com", mods);
  ...
  public boolean groupExists(String groupName, DirContext connection) {
    try {
      ...
      NamingEnumeration<SearchResult> results = connection.search("ou=Grupo,dc=ramhlocal,dc=com", filter,
          searchControls);
  ...
  public void modifyXML(String xmlFile, DirContext connection) {
    try {
      ...
      for (int i = 0; i < modifyNodes.getLength(); i++) {
        ...
        NodeList removeNodes = (NodeList) xpath.evaluate("modify-attr[@attr-name='Grupo']/remove-value/value",
            modifyNode, XPathConstants.NODESET);
        NodeList addNodes = (NodeList) xpath.evaluate("modify-attr[@attr-name='Grupo']/add-value/value",
            modifyNode, XPathConstants.NODESET);
